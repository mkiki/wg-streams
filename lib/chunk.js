...
Chunk.prototype.readZStringUTF16 = function(allowShortRead) {
  ...
  while ((that.maxPosition - that.stream.position) > 1) {
    ...
    if (b1 === 0xFE && b2 === 0xFF) throw new Exception("Big endian encoding not supported when reading UTF-16 string");
    if (b1 === 0x00 && b2 === 0x00) {
      return buffer.toString('ucs2',from, that.stream.position-2); 
    }
  }
  if (allowShortRead === true) return buffer.toString('ucs2',from, that.maxPosition); 
